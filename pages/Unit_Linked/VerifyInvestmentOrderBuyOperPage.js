...
// locators
const { search_verify_InvestmentOrderOper, table_verify_InvestmentOrderOper, dialog_verify_InvestmentOrderOper } = require('../../locators/Unit_Linked/VerifyInvestmentOrderBuyOper.locators.js');


class VerifyInvestmentOrderBuyOperPage {
    ...
    async search_verify_VerifyInvestmentOrderOper_Tab1(data) {
        // เลือกวันที่ส้งคำสั่ง
        await selectDate(this.page, data.date, this.search_verify_investmentorderoper.verify_investmentorderoper_btnDatePicker_Tab1);
        // กดปุ่ม ค้นหา
        await this.search_verify_investmentorderoper.verify_investmentorderoper_btnSearch_Tab1.click({ timeout: 10000 });
        // รอโหลดตาราง
        await this.page.waitForLoadState('networkidle');
    }
    ...
    async confirm_verify_VerifyInvestmentOrderOper_Tab1() {
        ...
        // รอ dialog ยืนยันคำสั่งขายขึ้นมา
        await this.expect(this.dialog_verify_investmentorderoper.verify_investmentorderoper_confirmorderinvestment_Tab1).toBeVisible({ timeout: 60000 });
        await this.dialog_verify_investmentorderoper.verify_investmentorderoper_confirmorderinvestment_Tab1.getByText('ใช่', { exact: true }).click({ timeout: 10000 });
        // รอ dialog หายไป
        await this.expect(this.dialog_verify_investmentorderoper.verify_investmentorderoper_confirmorderinvestment_Tab1).not.toBeVisible({ timeout: 60000 });
        ...
        // รอ popup ยืนยันรายการคำสั่งขาย เรียบร้อย ขึ้นมา
        await this.expect(this.dialog_verify_investmentorderoper.verify_investmentorderoper_successpopup_Tab1).toBeVisible({ timeout: 60000 });
        await this.dialog_verify_investmentorderoper.verify_investmentorderoper_successpopup_Tab1.getByText('ตกลง', { exact: true }).click({ timeout: 10000 });
        // รอ popup หายไป
        await this.expect(this.dialog_verify_investmentorderoper.verify_investmentorderoper_successpopup_Tab1).not.toBeVisible({ timeout: 60000 });
    }
}

module.exports = { VerifyInvestmentOrderBuyOperPage };