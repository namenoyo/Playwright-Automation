...
caseDatas.forEach((data, idx) => {
  test(`Automate เชคเบี้ย ชุดที่ ${idx + 1} `, async ({ page }) => {
    ...
    const actualPremiumAddition = normalizeNumber(tabledataobj[2].insurance_premium_addition);
    const expectedPremiumAddition = normalizeNumber(data.premiumAddition);
    ...
    try {
      expect(actualPremium).toBe(expectedPremium);
      console.log(`✅ expect CPAX_premium: "${actualPremiumAddition}" === "${expectedPremiumAddition}" (ผ่าน)`);
    } catch (e) {
      console.log(`❌ expect CPAX_premium: "${actualPremiumAddition}" !== "${expectedPremiumAddition}" (ไม่ผ่าน)`);
      throw e;
    }
    ...
    const result = {
      ...
      actualPremiumAddition,
      premiumResultAddition: actualPremium === expectedPremium ? 'PASS' : 'FAIL',
      expectCommission,
      actualCommission,
