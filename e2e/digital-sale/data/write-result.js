...
const SHEET_NAME = 'Claude_Prepare_Test_Data_DigitalSales';
  ...
  return dataRows
    ...
    .map((row) => {
      ...
      return {
        ...
        linkProduct: getValue(row, headerMap, 'Link_Product'),
...
// ─── QR Payment Flow Functions ────────────────────────────────────────────────

function isQrRunnableRow(row, headerMap, createByFilter = '') {
  const no          = getValue(row, headerMap, 'No');
  const valid       = getValue(row, headerMap, 'Valid').toUpperCase();
  const createBy    = getValue(row, headerMap, 'Create By');
  const qrStatus    = getValue(row, headerMap, 'Test Status QR').toLowerCase();
  const applicationNo = getValue(row, headerMap, 'เลขใบคำขอ');
  ...
  return (
    ...
    createBy === createByFilter &&
    applicationNo !== '' &&
    (qrStatus === 'ready for test' || qrStatus === 'ready for retest')
  );
}

async function fetchQrRunnableCases(createByFilter) {
  if (!createByFilter) throw new Error('❌ ต้องส่ง createByFilter');
  const { dataRows, headerMap } = await loadSheet(true);

  return dataRows
    .filter(row => isQrRunnableRow(row, headerMap, createByFilter))
    .map(row => ({
      no:            getValue(row, headerMap, 'No'),
      environment:   getValue(row, headerMap, 'Env'),
      applicationNo: getValue(row, headerMap, 'เลขใบคำขอ'),
      ...
      premium: String(getRawValue(row, headerMap, 'เบี้ย/ทุนชดเชย')).replace(/,/g, '').replace(/\.00$/, '').trim(),
    }));
}

async function claimQrCase(no, createByFilter) {
  const sheets = await getSheetsClient();
  const { dataRows, headerMap } = await loadSheet(true);
  const rowNumber = await getRowNumberByNo(no);
  const row = dataRows[rowNumber - HEADER_ROW - 1];

  if (!isQrRunnableRow(row, headerMap, createByFilter)) {
    console.log(`⏭️ Skip QR No ${no}`);
    return false;
  }

  const qrColIdx = headerMap.get('Test Status QR');
  if (qrColIdx === undefined) throw new Error('ไม่พบ column "Test Status QR" ใน sheet');

  await sheets.spreadsheets.values.update({
    spreadsheetId: SPREADSHEET_ID,
    range: `${SHEET_NAME}!${columnToLetter(qrColIdx + 1)}${rowNumber}`,
    valueInputOption: 'USER_ENTERED',
    requestBody: { values: [['Inprogress']] },
  });

  console.log(`🏃 QR Claim No ${no} | เลขใบคำขอ ${getValue(row, headerMap, 'เลขใบคำขอ')}`);
  return true;
}

async function writeQrResult({ no, status, remark, receiptNo = '', policyNo = '' }) {
  const sheets = await getSheetsClient();
  const { headerMap } = await loadSheet();
  const rowNumber = await getRowNumberByNo(no);
  ...
  const nextStatus = status === 'PASS' ? 'Done' : 'Ready for Retest';
  const testDate = `'${new Date().toLocaleString('sv-SE', { timeZone: 'Asia/Bangkok', hour12: false }).replace('T', ' ')}`;

  const data = [
    { range: `${SHEET_NAME}!${columnToLetter(qrColIdx + 1)}${rowNumber}`, values: [[nextStatus]] },
    { range: getA1Cell(rowNumber, headerMap, 'Remark'),    values: [[remark]]   },
    { range: getA1Cell(rowNumber, headerMap, 'Test Date'), values: [[testDate]] },
  ];

  if (receiptNo) data.push({ range: getA1Cell(rowNumber, headerMap, 'เลขใบเสร็จ'),    values: [[receiptNo]] });
  if (policyNo)  data.push({ range: getA1Cell(rowNumber, headerMap, 'เลขกรมธรรม์'), values: [[policyNo]]  });
