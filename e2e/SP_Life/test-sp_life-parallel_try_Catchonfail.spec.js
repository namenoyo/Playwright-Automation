...
test.describe.only('SP Life Insurance Premium Calculation Tests', () => {
    ...
    const ranges = [
        // { start: 7, end: 206 },    // ช่วงที่ 1
        // { start: 207, end: 406 },  // ช่วงที่ 2
        // { start: 407, end: 606 },  // ช่วงที่ 3
        // { start: 607, end: 806 },  // ช่วงที่ 4
        // { start: 807, end: 1006 }, // ช่วงที่ 5
        // { start: 1007, end: 1206 }, // ช่วงที่ 6
        // { start: 1207, end: 1406 }, // ช่วงที่ 7
        // { start: 1407, end: 1606 }, // ช่วงที่ 8
        // { start: 1607, end: 1806 }, // ช่วงที่ 9
        { start: 1807, end: 2006 }, // ช่วงที่ 10  เนมลองรัน
        // { start: 2007, end: 2206 }, // ช่วงที่ 11
        // { start: 2207, end: 2406 }, // ช่วงที่ 12
        // { start: 2407, end: 2606 }, // ช่วงที่ 13
        // { start: 2607, end: 2806 }, // ช่วงที่ 14
        // { start: 2807, end: 3006 }, // ช่วงที่ 15
        // { start: 3007, end: 3206 }, // ช่วงที่ 16
        // { start: 3207, end: 3406 }, // ช่วงที่ 17
        // { start: 3407, end: 3606 }, // ช่วงที่ 18
        // { start: 3607, end: 3806 }, // ช่วงที่ 19
        // { start: 3807, end: 4006 }, // ช่วงที่ 20
    ];
    ...
    for (const [index, row] of ranges.entries()) {
        test(`Calculate Insurance Premium - Worker ${index + 1}`, async ({ page }) => {
            ...
            // เพิ่มตัวแปรเก็บ log error
            const errorLogs = [];

            for (let row = 0; row < data.length; row++) {
                let retryCount = 0;
                let success = false;
                while (retryCount < 2 && !success) { // retry 1 ครั้ง (รวม 2 รอบ)
                    try {
                        ...
                        success = true; // ถ้าไม่ throw error ถือว่าผ่าน
                    } catch (e) {
                        retryCount++;
                        // ตรวจสอบว่าเป็น timeout error หรือไม่ (Playwright จะ throw error ที่มีชื่อหรือข้อความเกี่ยวกับ timeout)
                        const isTimeout = e.name === 'TimeoutError' || (e.message && e.message.toLowerCase().includes('timeout'));
                        const logMsg = `Row ${row + 1} (retry ${retryCount})${isTimeout ? ' [TIMEOUT]' : ''}: ${e.message || e}`;
                        console.error(logMsg);
                        if (retryCount >= 2) {
                            errorLogs.push(logMsg);
                        } else {
                            // refresh หน้าใหม่ก่อน retry
                            await page.reload({ waitUntil: 'networkidle' });
                            // อาจต้อง login ใหม่และไปที่หน้าเดิม
                            await loginpagesplife.login(datalogin[0][0], datalogin[1][0]);
                            await page.waitForLoadState('networkidle');
                            await mainsplife.clickcreateQuotation();
                            await quotationsplife.waitforquotationPageLoad();
