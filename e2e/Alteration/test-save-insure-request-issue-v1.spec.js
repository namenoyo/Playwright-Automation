...
for (const data_save_endorse of testData) {
    ...
    const endorse_data = data_save_endorse.endorse; // ข้อมูลสลักหลังที่ต้องการเลือก ตัวอย่างสำหรับทดสอบ

    let endorse_code = '';
    for (const code of endorse_data) {
        // เช็คว่า ถ้าตัวสุดท้ายไม่ต้องใส่ ,
        if (code === endorse_data[endorse_data.length - 1]) {
            endorse_code += code.endorse_code;
        } else {
            endorse_code += code.endorse_code + ',';
        }
    }

    test(`Scenario | ${data_save_endorse.channel_code}_${data_save_endorse.policy_type}_${data_save_endorse.policy_line}_${data_save_endorse.policy_status}_${data_save_endorse.contact_code} | ${endorse_code}`, async ({ page }, testInfo) => {
        ...
        if (policyNo === 'NO_POLICY_FOR_TEST') {
            ...
            try {
                ...
                for (const data_endorse of endorse_data) {
                    // กรอกข้อมูลสลักหลัง
                    await receiveissuerequestalteration.inputdataendorse_alteration(data_endorse);
                }
