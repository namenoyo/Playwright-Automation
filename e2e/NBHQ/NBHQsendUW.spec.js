...
import { caseDatas } from '../data/datacpax.js'; 

caseDatas.forEach((finalData, idx) => {
test(`test NBHQ ${idx + 1} `, async ({ page }) => {
  // Login NBS SIT or UAT
  await page.waitForTimeout(1000);
  if(finalData.environment == 'SIT'){
  await page.goto('https://sitnbs.thaisamut.co.th/nbsweb/secure/home.html');
  await page.locator('#username').click();
  await page.locator('#username').fill('0001'); // ยังคง Hardcode เนื่องจากข้อมูล Login ไม่ได้อยู่ใน NewCaseData
...
// --- ส่วนของการเรียกใช้และ Playwright ---
...
await page.getByRole('textbox', { name: 'วันเกิด *' }).press('Tab');
  ...
  await page.locator('#section-main #applicationNo').fill(finalData.applicationNo);
  await page.locator('#section-main #applicationNo').press('Tab');
  await page.waitForTimeout(5000);
  ...
  // สมมติว่าต้องการ Hardcode วันที่บัตรหมดอายุ หรือเพิ่มฟิลด์นี้ใน NewCaseData
...
console.log(`เบอร์โทรศัพท์ที่ใช้ในการทดสอบ: ${randomPhone}`);
...
//await page.getByRole(00'textbox', { name: 'อีเมล *' }).fill('testdataone@gmail.com');
await page.waitForTimeout(1000);
  ...
  // // สมมติว่าต้องการ Hardcode การส่งเอกสารกรมธรรม์ หรือเพิ่มฟิลด์นี้ใน NewCaseData
  ...
  //แก้ตาม A18
if (['A18', 'A19', 'A20'].includes(finalData.policyName)) {
 // Category แบบประกัน
  ...
  console.log('⏩ No Close button found within 3 seconds, continue...');
  }
  ...
  //await page.getByText('เบี้ยประกันภัยรวม').nth(1).fill(finalData.insuredAmount);
  }
  ...
  await page.getByRole('textbox', { name: 'ชื่อแบบประกันภัย *' }).press('Tab');
  await page.waitForTimeout(3000);  
  await page.getByRole('textbox', { name: 'จำนวนเงินเอาประกันภัย *' }).fill(finalData.insuredAmount);
...
//if(finalData.numOfRider!=='0'){
let numRider = 0;
// เพิ่มสัญญาเพิ่มเติมจาก Array ใน newCaseExample.riders
for (const rider of finalData.riders) {
  ...
  await page.getByRole('textbox', { name: 'ชื่อสัญญาเพิ่มเติม *' }).fill(rider.name);
  await page.getByRole('textbox', { name: 'ชื่อสัญญาเพิ่มเติม *' }).press('Tab');
  await page.waitForTimeout(1000);

  await page.getByText('กรุณาเลือก').click();
  await page.getByRole('textbox', { name: 'ทุนประกันภัย/แผนความคุ้มครอง *' }).fill(rider.coverage);
  await page.getByRole('textbox', { name: 'ทุนประกันภัย/แผนความคุ้มครอง *' }).press('Tab');
  await page.waitForTimeout(1500);

  await page.getByRole('button', { name: 'เพิ่ม' }).click();
  await page.waitForTimeout(2000); // รอหลังจากเพิ่มแต่ละ Rider
  
  // นับจำนวนสัญญาเพิ่มเติมที่ถูกเพิ่ม
  numRider++;
}

// ตอนนี้ตัวแปร numRider จะมีค่าเท่ากับจำนวนสัญญาเพิ่มเติมที่เพิ่มเข้าไป
console.log(`จำนวนสัญญาเพิ่มเติมที่เพิ่ม: ${numRider} รายการ`);
//}
//ตรวจสอบค่าเบี้ยที่แสดงในตารางทั้งหมด
...
// 1. ค้นหา index ของแบบประกันหลักจากชื่อใน newCaseExample
insuMainIndex = draftPremium3.findIndex(text => text.includes(finalData.policyName));

if (insuMainIndex !== -1) {
  // ดึงค่าสำหรับแบบประกันหลัก (Main Insurance)
  const insuMain = draftPremium3[insuMainIndex];
  const insuMoney = draftPremium3[insuMainIndex + 4];
  const insuPremium = draftPremium3[insuMainIndex + 6];
  const insuSpePremium = draftPremium3[insuMainIndex + 8];
  const insuCommis = draftPremium3[insuMainIndex + 12];

  // สร้างอาร์เรย์สำหรับเก็บสัญญาเพิ่มเติม (Riders)
  const riders = [];
  let lastRiderIndex = insuMainIndex; // ใช้เพื่อติดตามตำแหน่งของ rider ตัวล่าสุด

  // 2. วนลูปเพื่อค้นหาและดึงข้อมูลของสัญญาเพิ่มเติมแต่ละตัว
  for (const riderConfig of finalData.riders) {
    const riderNameIndex = draftPremium3.findIndex((text, index) => {
      ...
      return index > lastRiderIndex && text.includes(riderConfig.name);
    });
    ...
    if (riderNameIndex !== -1) {
      const rider = {
        ...
        premium: draftPremium3[riderNameIndex + 6],
        spePremium: draftPremium3[riderNameIndex + 8],
        commis: draftPremium3[riderNameIndex + 12],
      };
      riders.push(rider);
      lastRiderIndex = riderNameIndex; // อัปเดตตำแหน่งของ rider ตัวล่าสุด
    }
  }
...
if (premiumPayKeywordIndex !== -1) {
    // ถ้าหาเจอ ให้คำนวณ index ที่ถูกต้องโดยบวก 6
    const premiumPayIndex = premiumPayKeywordIndex + 6;

    // ตรวจสอบว่า index ที่คำนวณได้ยังอยู่ในขอบเขตของ Array
    if (premiumPayIndex < draftPremium3.length) {
        premiumPay = draftPremium3[premiumPayIndex];
    } else {
        premiumPay = 'ไม่พบข้อมูลเนื่องจาก Index อยู่นอกขอบเขต';
    }
} else {
  ...
  console.log('---');
  console.log(`ค่า Premium ที่ชำระ: ${premiumPay}`);
...
// Category จัดการผู้รับผลประโยชน์
  ...
  await page.getByRole('textbox', { name: 'ความสัมพันธ์ *' }).fill(finalData.beneRela_1);
  await page.getByRole('textbox', { name: 'ความสัมพันธ์ *' }).press('Tab');
  await page.waitForTimeout(500);

  await page.getByRole('textbox', { name: 'คำนำหน้า *' }).click();
  await page.getByRole('textbox', { name: 'คำนำหน้า *' }).fill(finalData.benePrefix_1);
  await page.getByRole('textbox', { name: 'คำนำหน้า *' }).press('Tab');
  await page.waitForTimeout(500);

  await page.getByRole('textbox', { name: 'ชื่อ *' }).click();
  await page.getByRole('textbox', { name: 'ชื่อ *' }).fill(finalData.beneName_1);
  await page.waitForTimeout(500);

  await page.getByLabel('จัดการผู้รับประโยชน์').locator('div').filter({ hasText: /^นามสกุล$/ }).nth(1).click();
  await page.getByRole('textbox', { name: 'นามสกุล', exact: true }).fill(finalData.beneSurname_1);
  await page.waitForTimeout(500);

  await page.getByRole('textbox', { name: 'อายุ *' }).click();
  await page.getByRole('textbox', { name: 'อายุ *' }).fill(finalData.beneAge_1);
  await page.waitForTimeout(500);
...
// const numOfBebeInt =''
// numOfBebeInt = finalData.numBene.parse int แปลงค่าจาก structure จาก string มาเป็น int ก่อนเพื่อมาคำนวณ 
const numBeneInt = parseInt(finalData.numBene, 10);
  if(numBeneInt === 2){
    ...
    await page.getByRole('textbox', { name: 'ความสัมพันธ์ *' }).click();
    await page.getByRole('textbox', { name: 'ความสัมพันธ์ *' }).fill(finalData.beneRela_2);
    await page.getByRole('textbox', { name: 'ความสัมพันธ์ *' }).press('Tab');
    await page.waitForTimeout(500);

    await page.getByRole('textbox', { name: 'คำนำหน้า *' }).click();
    await page.getByRole('textbox', { name: 'คำนำหน้า *' }).fill(finalData.benePrefix_2);
    await page.getByRole('textbox', { name: 'คำนำหน้า *' }).press('Tab');
    await page.waitForTimeout(500);

    await page.getByRole('textbox', { name: 'ชื่อ *' }).click();
    await page.getByRole('textbox', { name: 'ชื่อ *' }).fill(finalData.beneName_2);
    await page.waitForTimeout(500);

    await page.getByLabel('จัดการผู้รับประโยชน์').locator('div').filter({ hasText: /^นามสกุล$/ }).nth(1).click();
    await page.getByRole('textbox', { name: 'นามสกุล', exact: true }).fill(finalData.beneSurname_2);
    await page.waitForTimeout(500);

    await page.getByRole('textbox', { name: 'อายุ *' }).click();
    await page.getByRole('textbox', { name: 'อายุ *' }).fill(finalData.beneAge_2);
    await page.waitForTimeout(500);
    await page.getByText('เฉลี่ยสัดส่วนผลประโยชน์เท่าๆกัน').first().click();
    await page.waitForTimeout(1500);
 ...
 //จับจาก block ที่ 3ก่อน inspect แล้วให้จับ unique มากที่สุด ตาม label
...
await page.goto('https://sitnbs.thaisamut.co.th/nbsweb/secure/home.html');
  ...
  await page.waitForTimeout(200);
    ...
    //await page.waitForTimeout(1000);    
    await page.locator('.MuiGrid-root.MuiGrid-container.MuiGrid-spacing-xs-1 > div:nth-child(3) > .MuiFormControl-root > .MuiInputBase-root > .MuiInputBase-input').click();
    await page.locator('.MuiInputBase-root.MuiInput-root.MuiInput-underline.MuiInputBase-fullWidth.MuiInput-fullWidth.Mui-focused > .MuiInputBase-input').fill((premiumPay ?? 0).toString()); //ค่าเบี้ยที่ไปดึงมาจากหน้าก่อน
    await page.waitForTimeout(500);
    ...
    await page.getByText('เลือก').first().click();
    await page.locator('#policyTypeSearch').nth(1).fill(finalData.policyType); 
    ...
    await page.getByRole('button', { name: 'ยืนยัน' }).click();
    await page.getByText('เลือก').first().click();
    await page.locator('#planCode').nth(1).fill(finalData.policyName); //ใส่แบบประกัน
    ...
    // await page.waitForTimeout(1000);
    await page.locator('.MuiInputBase-root.MuiInput-root.MuiInput-underline.MuiInputBase-fullWidth.MuiInput-fullWidth.Mui-focused > .MuiInputBase-input').fill((premiumPay ?? 0).toString()); //ใส่จนเงิน  premiumAll
    await page.locator('.MuiInputBase-root.MuiInput-root.MuiInput-underline.MuiInputBase-fullWidth.MuiInput-fullWidth.Mui-focused > .MuiInputBase-input').press('Tab');
    await page.waitForTimeout(1000);
    await page.getByText('เลือก').click();
  ...
  // await page.locator('.MuiInputBase-root.MuiInput-root.MuiInput-underline.MuiInputBase-fullWidth.MuiInput-fullWidth.Mui-focused > .MuiInputBase-input').press('Tab');
  // await page.waitForTimeout(1000);
  await page.locator('.MuiInputBase-root.MuiInput-root.MuiInput-underline.MuiInputBase-fullWidth.MuiInput-fullWidth.Mui-focused > .MuiInputBase-input').fill((premiumPay ?? 0).toString());
  await page.locator('.MuiInputBase-root.MuiInput-root.MuiInput-underline.MuiInputBase-fullWidth.MuiInput-fullWidth.Mui-focused > .MuiInputBase-input').press('Tab');
  await page.waitForTimeout(1000);
...
//ตรวจสอบการบันทึกข้อมูลเคสใหม่
 if(finalData.environment == 'SIT'){
   ...
   await page.getByRole('textbox', { name: 'เลขที่ใบคำขอ' }).click();
   await page.getByRole('textbox', { name: 'เลขที่ใบคำขอ' }).fill(finalData.applicationNo); //เลขที่ใบคำขอ
   await page.getByRole('textbox', { name: 'เลขที่ใบคำขอ' }).press('Tab');
   await page.getByText('ทั้งหมด').nth(1).click();
   await page.getByRole('textbox', { name: 'ประเภทกรมธรรม์' }).fill('สามัญ'); // ประเภทกรมธรรม์
   await page.getByRole('textbox', { name: 'ประเภทกรมธรรม์' }).press('Tab');

   await page.getByText('ทั้งหมด').nth(1).click();
   if (finalData.partnerNo === '8500001') {
   await page.getByText('8500001-Digital Sales').first().click();
 } else if (finalData.partnerNo === '8100001') {
   await page.getByText('8100001-ฝ่ายธุรกิจสถาบัน').first().click();
 } else if (finalData.partnerNo === '8290001') {
   await page.getByText('8290001-Worksite (ALT2)').first().click();
 } else {
   console.log(`ไม่พบเงื่อนไขสำหรับ partnerNo: ${finalData.partnerNo} ไม่มีการคลิก`);
 }

 await page.waitForTimeout(200);

 await page.getByText('ค้นหา').nth(1).click();
 await page.waitForTimeout(3000);

 await page.getByRole('button', { name: 'ﻃ' }).click();
 await page.waitForTimeout(4000);


 await page.getByRole('button', { name: 'ยืนยันตรวจสอบข้อมูล' }).click();
 await page.waitForTimeout(500);


//เผื่อมี pop up เด้งขึ้นมา เช่น คำนำหน้าไม่ตรงกัน
try {
  const closeBtn2 = page.getByRole('button', { name: 'ปิด' });
  await closeBtn2.waitFor({ state: 'visible', timeout: 3000 });
...
try {
  const closeBtn3 = page.getByRole('button', { name: 'ตกลง' });
  await closeBtn3.waitFor({ state: 'visible', timeout: 3000 });
...
try {
  const closeBtn4 = page.getByRole('button', { name: 'ยืนยัน' });
  await closeBtn4.waitFor({ state: 'visible', timeout: 3000 });
  ...
  try {
  const closeBtn9 = page.getByRole('button', { name: 'ปิด' });
  await closeBtn9.waitFor({ state: 'visible', timeout: 3000 });
  await closeBtn9.click();
   console.log('✅ Clicked Close button');
  } 
  catch {
  console.log('⏩ No Close button found within 3 seconds, continue...');
