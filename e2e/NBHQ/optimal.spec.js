...
caseDatas.forEach((finalData, idx) => {
  test(`test NBHQ ${idx + 1} `, async ({ page }) => {
    ...
    const startTime = Date.now(); // ⏱️ เวลาเริ่มต้น
    ...
    if (finalData.environment == 'SIT') {
      ...
      await page.locator('#password').fillbra('12');
    ...
    //ไม่ต้องตรวจสอบ เพราะมันพังอยู่ 
    ...
    //    console.log('✅ Clicked ปิด button');
    //   } 
    //   catch {
    //   console.log('⏩ No ปิด button found within 3 seconds, continue...');
    //   }
    // try {
    //   const closeBtn3 = page.getByRole('button', { name: 'ตกลง' });
    //   await closeBtn3.waitFor({ state: 'visible', timeout: 3000 });
    //   await closeBtn3.click();
    //    console.log('✅ Clicked ตกลง button');
    //   } 
    //   catch {
    //   console.log('⏩ No Close ตกลง found within 3 seconds, continue...');
    //   }
    // try {
    //   const closeBtn4 = page.getByRole('button', { name: 'ยืนยัน' });
    //   await closeBtn4.waitFor({ state: 'visible', timeout: 3000 });
    //   await closeBtn4.click();
    //    console.log('✅ Clicked ยืนยัน button');
    //   } 
    //   catch {
    //   console.log('⏩ No ยืนยัน button found within 3 seconds, continue...');
    //   }
    //   try {
    ...
    const elapsedSec11 = ((Date.now() - startTime) / 1000).toFixed(2);
    console.log(`⏱️ ใช้เวลาไป ${elapsedSec11}s`);
  });
});