...
function tryBecomeReportWriter() {
  ...
  try {
    const fd = fs.openSync(reportWriterFile, 'wx'); // 'wx' = fail ถ้าไฟล์มีอยู่
    fs.writeSync(fd, String(Date.now()));          // ✅ เขียนผ่าน fd
    fs.closeSync(fd);
    return true;
  } catch {
    return false;
...
// รอจน done ครบ expected (poll แบบเบาๆ) — เสถียรกว่าเดิม
...
if (process.stderr.isTTY) {
  ...
  readline.cursorTo(process.stderr, 0);
      ...
      process.stderr.write(`✅ Completed workers: ${String(count).padStart(digits,'0')}/${String(expected).padStart(digits,'0')}`);
    }
    ...
    if (Date.now() - start > timeoutMs) {
      process.stderr.write('\n');
      throw new Error(`waitForAllFinished timeout: ${count}/${expected}`);
    }
...
// Test data
// ==============================
const phonenos = ['0874787214','0933995002','0933995003']; //,'0933995004','0933995005','0933995006','0933995007','0933995008','0933995009','0933995010','0933995011','0933995012','0933995013','0933995014','0933995015','0933995016','0933995017','0933995018','0933995019','0933995020'
...
// console.log(`Worker ${seq} : ${refcode ? refcode.trim() : ''}`);
      ...
      try { stopKA && stopKA(); } catch {} finally { stopKA = null; }
...
test.afterEach(async ({}, testInfo) => {
  const m = testInfo.title.match(/(\d+)\s*$/);
  const seq = m ? parseInt(m[1], 10) : null;
  if (seq) {
    try { safeAppendDone(finishedFile, seq); } catch {}
  }
});
...
test.afterAll(async () => {
  // ยืดเวลา hook (ดีฟอลต์แค่ ~30s)
  test.setTimeout(30 * 60_000);

  // ⬅️ เพิ่ม guard ตรงนี้
  if (!tryBecomeReportWriter()) return;

  const expectedReady = Math.min(totalWorkers || phonenos.length, phonenos.length);

  // ❗ อย่าบล็อกไฟล์ทั้งไฟล์ — ถ้ารอครบก็สรุป, ถ้าไม่ครบใน 12s ก็ข้ามไป (ไม่ throw)
  try {
    await Promise.race([
      waitForAllFinished(expectedReady, { timeoutMs: 25 * 60_000, pollMs: 500 }),
      sleep(12_000),
    ]);
  } catch (e) {
    // กันไว้เฉย ๆ: ไม่โยน error ออกไป เพื่อไม่ให้ไฟล์ fail เพราะ hook
    console.warn(`[report] skip waiting all finished: ${e?.message || e}`);
  }

  // ===== อ่านผลและสรุป (best-effort) =====
  // แทนบล็อกอ่านไฟล์สรุปใน afterAll
let summaryResults = [];
try {
  if (fs.existsSync(resultsDir)) {
    ...
    for (const f of files) {
      try {
        ...
        byId.set(r.workerId, r);  // โอเวอร์ไรด์ด้วยอันล่าสุด
      } catch {}
    }
    summaryResults = Array.from(byId.values()).sort((a,b) => a.workerId - b.workerId);
  }
} catch {}

// 📊 ตรวจเตือนถ้าจำนวน done ไม่ครบก่อนสรุป
const doneLines = fs.existsSync(finishedFile)
  ? fs.readFileSync(finishedFile, 'utf8').trim().split('\n').filter(Boolean)
  : [];
const doneCount = new Set(doneLines).size;
if (doneCount !== expectedReady) {
  console.warn(`⚠️ Warning: done workers = ${doneCount}/${expectedReady} (บาง worker อาจพังหรือ retry ไม่สำเร็จ)`);
}
  ...
  console.log('\n🧪━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('📊 FINAL OTP TEST REPORT (best-effort)');
  ...
  console.log(`\n✅ Total Workers (reported): ${summaryResults.length}`);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
});
